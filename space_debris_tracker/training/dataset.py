...
class DebrisImageDataset(Dataset):
    ...
    def __init__(
        ...
        self.class_names = class_names or [
            'debris', 'satellite', 'rocket_body', 'fragment'
        ]
...
class OrbitDataset(Dataset):
    ...
    def _load_data(self):
        ...
        for traj_idx in range(len(self.trajectories)):
            ...
            for start_idx in range(0, traj_len - self.sequence_length - self.prediction_horizon, self.stride):
...
class ConjunctionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            'collision_probability': torch.tensor(event['collision_probability'], dtype=torch.float32),
            'risk_label': torch.tensor(1 if event['collision_probability'] > 1e-4 else 0, dtype=torch.long)
        }
