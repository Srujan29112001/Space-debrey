...
class SpaceMonitoringAgent:
    ...
    def __init__(self,
        ...
        self.mcp_client = MCPClient(
            agent_id=f"sat_{satellite_id}",
            broker_url="tcp://localhost:5555"
        )

        # Observation scheduler (RL-based)
        self.observation_scheduler = ObservationScheduler(
    ...
    async def continuous_monitoring(self):
        ...
        while True:
            try:
                ...
                high_risk = [c for c in conjunctions
    ...
    async def predict_conjunction(self,
                                 sat_id: int,
                                 obj_id: str,
        ...
        for msg in messages:
            if msg.get('type') == 'conjunction_update':
                ...
                for conj in msg.get('conjunctions', []):
                    if conj['primary'] == self.satellite_id or conj['secondary'] == f"SAT_{self.satellite_id}":
    ...
    async def calculate_avoidance_maneuver(self,
        ...
        result = minimize(
            ...
            method='SLSQP',
            constraints=[
                {'type': 'ineq', 'fun': constraint_miss_distance}
            ],
        ...
        return {
            ...
            'direction': (delta_v / np.linalg.norm(delta_v)).tolist() if np.linalg.norm(delta_v) > 0 else [0, 0, 0],
